...
export const States: React.FC = () => {
  ...
  const loadTree = useCallback(async (path?: string) => {
    ...
    try {
      ...
      const response = await fetch(`/api/states/tree${params}`);
  ...
  const loadChildren = useCallback(async (node: FileNode): Promise<FileNode[]> => {
    ...
    try {
      const response = await fetch(`/api/states/tree?path=${encodeURIComponent(node.path)}`);
      const data = await response.json();
  ...
  if (loading && !treeData) {
    return <div className="states-loading">加载中...</div>;
  }
  ...
  if (error && !treeData) {
    ...
    return (
      <div className="states-error">
        <h3>加载失败</h3>
        <p>{isNotFound ? 'States 目录不存在，请确认 OpenClaw 配置' : error}</p>
  ...
  return (
    <div className="states-container">
      <div className="states-sidebar">
        ...
        {treeData && (
          <>
            <div className="states-path-info">
              <div className="states-path-header">
                <span className="states-path-label">📂</span>
                <span>根目录</span>
              </div>
              <div className="states-path" title={treeData.absolutePath}>
                {treeData.absolutePath}
              </div>
            </div>
            {treeData.path !== '.' && (
              <div className="states-current-path">
                <span>📍 当前：</span>
