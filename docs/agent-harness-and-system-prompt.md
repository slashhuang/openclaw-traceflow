...
OpenClaw 另在 **Agent Runtime** 中描述引导文件职责（`external-refs/openclaw/docs/concepts/agent.md` — _Bootstrap files (injected)_）；与 **system-prompt** 中 **Workspace bootstrap injection** 一并阅读时，以 **system-prompt** 对「写入 Project Context 的文件清单与频率」的表述为准做信息架构。

### 4.2 System prompt 固定区块（OpenClaw _Structure_）
...
### 4.4 写入 **Project Context** 的工作区引导文件（OpenClaw _Workspace bootstrap injection_）
...
- `USER.md`
- `HEARTBEAT.md`
- `BOOTSTRAP.md`（仅全新工作区 _brand-new workspaces_）
...
### 4.5 Skills 在 system prompt 中的形态（OpenClaw _Skills_）
...
### 4.6 工作区标准文件表（OpenClaw _Agent workspace — Workspace file map_）

除 §4.4 所列注入文件外，OpenClaw 在工作区文档中还定义下列**标准路径**（含义以官方 _Workspace file map_ 为准；**含可选项**）：
...
| **P3（可选）** | 若产品定位为「不只读」：编辑草稿、与 workspace 同步策略、权限与审计                                                                                                                                     | 必须单开 **边界与风险**（写路径、冲突、合规）；并参考本仓库 **README**（_Gateway scopes_）与 **[CLAUDE.md](../CLAUDE.md)** 中 Gateway **backend 无设备时 scopes** 等约束，避免「远程改 prompt」假设脱离真实鉴权。 |
...
- Gateway 连接形态、无设备 backend 的 scope 行为等，以本仓库 **README / CLAUDE.md**（_Gateway scopes_）为准；设计跨服务写操作前必读。
