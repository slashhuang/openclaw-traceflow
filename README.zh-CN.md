...
面向 **[OpenClaw](https://docs.openclaw.ai) Agent** 的**可观测性** Web 应用：会话、Skill、Token 用量与告警、延迟（P50/P95/P99）、**Agent 与 Harness**（Project Context / OpenClaw Structure 等）、模型计价与实时日志。独立 NestJS + React 服务，界面支持**中文 / English**。
...
| **无 `operator.read` 时仍可安全概览 Gateway** | 不适用 | **有** — 路径探测用 connect 快照；无设备 backend 连接清空 scopes 时，仪表盘 health/overview 走 **`health` RPC**（豁免 scope 拦截）（见 monorepo 根目录 **`AGENTS.md`**） |
...
> 当前仓库快照未包含 `docs/traceFlowSnapshots/` 下的截图资源。  
> 本节结构保留，后续会补充与现版本一致的截图。
