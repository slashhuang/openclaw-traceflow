...
- **参与者列**优先展示索引或解析得到的**对接身份**（例如飞书 `open_id`）。消息正文里可能出现 `Sender (untrusted metadata)` 等块，TraceFlow 会解析到**单条消息的 `sender`**；若索引侧已有真实身份，列表会优先显示该身份，而不会仅因会话带系统标签就用 `heartbeat`/`cron`/`boot` 覆盖。
