...
export default {
  ...
  'header.gatewayClickToSettings': 'Click the Gateway status tag to open Gateway settings.',
  ...
  'settings.im': 'IM Push Configuration',
  'watchSession.title': 'Session Watch Status',
  'watchSession.activeCount': 'Active Sessions: {count}',
  'watchSession.noActive': 'No active sessions',
  'watchSession.watching': 'Watching',
  'watchSession.notWatching': 'Not Watching',
  ...
  'settings.configPath': 'openclaw.json path (optional, OPENCLAW_CONFIG_PATH)',
  'settings.bootstrapOverrides': 'Bootstrap file path overrides (JSON)',
  'settings.bootstrapOverridesHelp':
    'Map logical filenames to absolute paths, e.g. {"AGENTS.md":"/path/to/AGENTS.md"}. Leave empty or {} for no overrides.',
  ...
  'dashboard.agentCardSessions': 'Session entries',
  'dashboard.agentOverview': 'Sessions by agent',
  'dashboard.agentOverviewDesc':
    'Aggregated from disk under agents/<agentId>/sessions/. Click a card to open that agent’s session list.',
  'dashboard.recentByAgent': 'Recent sessions per agent',
  'dashboard.recentByAgentDesc': 'Up to 5 per agent by last activity (disk jsonl).',
  'dashboard.recentSessions': 'Recent sessions (global)',
  'dashboard.recentSessionsDesc':
    'Latest 10 sessions by last activity (all agents). “Recorded” is merged usage + utilization; “Est.(log)” is a log-size heuristic. Hover Status / Archived headers for definitions.',
  ...
  'sessions.filterByAgent': 'Filtered by agent: {agentId}',
  'sessions.clearAgentFilter': 'Clear filter',
  'sessions.landingByAgentTitle': 'Sessions by agent',
  'sessions.landingByAgentDesc':
    'Same metrics as the dashboard (GET /api/sessions/agent-overview). Choose an agent to open its paged list.',
  'sessions.agentSummaryHint': 'On-disk counts for this agent (same as the dashboard card).',
  'sessions.title': 'Sessions',
  'sessions.pageScopeDesc':
    'Data from GET /api/sessions (filter + paging match the request). “Recorded” and “Est.(log)” are separate columns; hover headers for scope. Default landing is the per-agent overview (aligned with the dashboard).',
  'sessions.sortHint': 'Click column header to sort (server-side ordering for paged results)',
  'sessions.column.type': 'Type',
  'sessions.column.chatKind': 'Shape',
  'sessions.chatKind.filterNone': 'None / system',
  'sessions.searchPlaceholder': 'Search session key, participants…',
  'sessions.agentSelectPlaceholder': 'Filter by agent (empty = all)',
  'sessions.column.agent': 'Agent',
  'sessions.allAgentsHint':
    'Showing sessions for all agents. Pick an agent above to narrow the list.',
  ...
  'systemPrompt.fullCollapseScopeDesc':
    'Advanced: full assembled Markdown from OpenClaw. Source may be transcript system, model reveal, or rebuild—see the alert when expanded.',
  'systemPrompt.breakdownCardScopeDesc':
    'Per-block tokens from probe.breakdown, mapped to OpenClaw Structure sections; sorted with Project Context / workspace injects (AGENTS.md, IDENTITY.md, …) first. Bar shows distribution. Panels start collapsed—expand for text or tables (Skills, tool schema stats, …).',
