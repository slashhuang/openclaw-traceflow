...
export class UserAccessToken {
    ...
    async initWithCode(
        key2Code: Record<string, string>,
        options?: { namespace?: string }
    ) {
        const key2Info: Record<string, ITokenInfo> = {};
        for (const [key, code] of Object.entries(key2Code)) {
            const oidcAccessInfo =
                await this.client.authen.oidcAccessToken.create({
                    data: {
            ...
            if (oidcAccessInfo.code !== 0) {
                ...
                this.client.logger.error(
                    'init user access token error',
                    key,
                    oidcAccessInfo.msg || oidcAccessInfo.message
                );
                continue;
            }
            // code expired
    ...
    async update(
        key2Info: Record<string, ITokenInfo>,
        options?: { namespace?: string }
    ) {
        for (const [key, info] of Object.entries(key2Info)) {
            ...
            const targetValue = mergeObject(cacheValue, {
                code,
                token,
                refreshToken,
                expiredTime,
    ...
    async get(key: string, options?: { namespace?: string }) {
        ...
        if (!cacheInfo) {
            this.client.logger.error(
                'user access token needs to be initialized or updated first'
            );
            return;
        }
        ...
        if (refreshToken) {
            const refreshAccessInfo =
                await this.client.authen.oidcRefreshAccessToken.create({
                    data: {
            ...
            if (refreshAccessInfo.code === 0 && refreshAccessInfo.data) {
                await this.update({
                    [key]: {
                        ...
                        expiredTime: this.calibrateTime(
                            refreshAccessInfo.data.expires_in
                        ),
                    },
                });
                ...
                this.client.logger.error(
                    'get user access token by refresh token failed.',
                    refreshAccessInfo.msg
                );
                return;
            }
        }
            ...
            if (oidcAccessInfo.code === 0 && oidcAccessInfo.data) {
                await this.update({
                    [key]: {
                        ...
                        refreshToken: oidcAccessInfo.data.refresh_token,
                        expiredTime: this.calibrateTime(
                            oidcAccessInfo.data.expires_in
                        ),
                    },
                });
            } else {
                this.client.logger.error(
                    'get user access token by code failed.',
                    oidcAccessInfo.msg
                );
            }
        }
