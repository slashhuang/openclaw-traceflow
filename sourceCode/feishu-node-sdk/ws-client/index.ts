...
export class WSClient {
    ...
    private async pullConnectConfig() {
        ...
        try {
            ...
            if (code !== ErrorCode.ok) {
                this.logger.error(
                    '[ws]',
                    `code: ${code}, ${code === ErrorCode.system_busy ? msg : 'system busy'}`
                );
                if (
            ...
            this.logger.debug(
                '[ws]',
                `get connect config success, ws url: ${URL}`
            );
    ...
    private async reConnect(isStart: boolean = false) {
        ...
        const tryConnect = () => {
            ...
            return this.pullConnectConfig()
                .then((isSuccess) =>
                    isSuccess ? this.connect() : Promise.resolve(false)
                )
                .then((isSuccess) => {
                    if (isSuccess) {
                        this.communicate();
        ...
        const reconnectNonceTime = reconnectNonce
            ? reconnectNonce * Math.random()
            : 0;
        this.reconnectInterval = setTimeout(async () => {
            (async function loopReConnect(this: WSClient, count: number) {
                ...
                this.logger.info(
                    'ws',
                    `unable to connect to the server after trying ${count} times")`
                );
                ...
                this.reconnectInfo.nextConnectTime =
                    Date.now() + reconnectInterval;
            }).bind(this)(0);
        }, reconnectNonceTime);
    ...
    private async handleControlData(data: pbbp2.Frame) {
        const type = data.headers.find(
            (item) => item.key === HeaderKey.type
        )?.value;
    ...
    private async handleEventData(data: pbbp2.Frame) {
        ...
        try {
            ...
            if (result) {
                respPayload.data = Buffer.from(JSON.stringify(result)).toString(
                    'base64'
                );
            }
        } catch (error) {
            ...
            this.logger.error(
                '[ws]',
                `invoke event failed, message_type: ${type}; message_id: ${message_id}; trace_id: ${trace_id}; error: ${error}`
            );
        }
        ...
        this.sendMessage({
            ...
            headers: [
                ...data.headers,
                { key: HeaderKey.biz_rt, value: String(startTime - endTime) },
            ],
    ...
    close(params: { force?: boolean } = {}) {
        ...
        this.logger.info(
            '[ws]',
            `ws client closed manually${force ? ' (force)' : ''}`
        );
    }
    ...
    async start(params: { eventDispatcher: EventDispatcher }) {
        ...
        if (!eventDispatcher) {
            this.logger.warn(
                '[ws]',
                'client need to start with a eventDispatcher'
            );
            return;
        }
