...
type TClientInstance = typeof Client extends new (...args: any[]) => infer R
    ? R
    : any;
type TPickParams<T extends (...args: any) => any> = Exclude<
    Parameters<T>['0'],
    undefined
>;
type TCreateSessionParams = TPickParams<
    TClientInstance['aily']['v1']['ailySession']['create']
>;
type TCreateMessageParams = TPickParams<
    TClientInstance['aily']['v1']['ailySessionAilyMessage']['create']
>;
type TCreateRunParams = TPickParams<
    TClientInstance['aily']['v1']['ailySessionRun']['create']
>;
type TMessage = ElementType<
    Exclude<
        Exclude<
            UnpackPromise<
                ReturnType<
                    TClientInstance['aily']['v1']['ailySessionAilyMessage']['list']
                >
            >['data'],
            undefined
        >['messages'],
...
interface ICreateParams {
    ...
    message: string;
    messageInfo?: Omit<
        TCreateMessageParams['data'],
        'idempotent_id' | 'content_type' | 'content'
    >;
    // run
    ailyAppId: string;
    skillId?: string;
...
export class Aily {
    ...
    private async getSessionId(params: {
        sessionId?: string;
        payload?: TCreateSessionParams['data'];
        ...
        const ailySession = await this.client.aily.v1.ailySession.create(
            { data: payload },
            options
        );
    ...
    private async waitReply(params: ICreateParams, options?: IRequestOptions) {
        ...
        const ailySessionId = await this.getSessionId({
            sessionId,
            payload: sessionInfo,
            options,
        });
        ...
        const ailySessionMessage =
            await this.client.aily.v1.ailySessionAilyMessage.create(
                {
                    path: {
        ...
        if (!(ailySessionMessage.code === 0 && ailySessionMessage.data)) {
            this.logger.error(
                'create aily message error',
                ailySessionMessage.msg
            );
            throw EExecStatus.ERROR;
        }
        ...
        const polling = async () => {
            return new Promise((resolve) => {
                setTimeout(async () => {
                    const runStatusInfo =
                        await this.client.aily.v1.ailySessionRun.get(
                            {
                                path: {
                                    aily_session_id: ailySessionId,
                                    run_id: runId,
                    ...
                    switch (status) {
                        ...
                        case 'COMPLETED':
                            resolve(EExecStatus.SUCCESS);
                        case 'EXPIRED':
                            resolve(EExecStatus.EXPIRED);
                        case 'CANCELLED':
                            resolve(EExecStatus.CANCELLED);
                        case 'FAILED':
                            resolve(EExecStatus.FAILED);
                        default:
                            resolve(EExecStatus.OTHER);
                    }
                }, 500);
            });
        };
    ...
    private async create(params: ICreateParams, options?: IRequestOptions) {
        const { ailySessionId, runId } = await this.waitReply(params, options);
    ...
    private async createWithStream(
        params: ICreateParams,
        options?: IRequestOptions
    ) {
        const { ailySessionId, runId, messageId } = await this.waitReply(
            params,
            options
        );

        const listMessages = () =>
            this.client.aily.v1.ailySessionAilyMessage.listWithIterator(
                {
                    path: {
                        aily_session_id: ailySessionId,
                    },
